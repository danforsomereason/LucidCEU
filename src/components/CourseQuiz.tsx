...
const CourseQuiz: React.FC<QuizProps> = ({ onQuizComplete, course }) => {
        ...
        if (!showFeedback) {
            ...
            if (currentQuestionIndex < course.questions.length - 1) {
                ...
                const score = Math.round(
                    (totalCorrectAnswers / course.questions.length) * 100
                );

                const currentAttempt =
                    (location.state as LocationState)?.attemptNumber || 1;
    ...
    return (
        <Paper sx={{ p: 4, maxWidth: 800, mx: "auto" }}>
            <Typography variant="h5" gutterBottom>
                Question {currentQuestionIndex} of {course.questions.length}
            </Typography>
            ...
            <FormControl component="fieldset">{renderOptions()}</FormControl>
